...
def keep_above_treshold(df,geometry,resolution=30,threshold_precent = 50,grouping_over =['Year','Month','Day','DOY'] ):

    num_of_pixels = int(geometry.area().getInfo()/(resolution**2))
    print(f"you have {num_of_pixels} pixels over the pasture")
    threshold = int((threshold_precent/100) * num_of_pixels)
    print(f"Threshold is clear {threshold} pixels")
    group_sizes = df.groupby(grouping_over).size()
    valid_groups = group_sizes[group_sizes >= threshold]
    valid_group_keys = valid_groups.index.tolist()
    filtered_df = df[df.set_index(grouping_over).index.isin(valid_group_keys)]
    return filtered_df
...
def to_numeric_features(df,features_list):
    for feature in features_list:
        df[feature] = pd.to_numeric(df[feature])


# add past sum of a feature
def add_past_sum(df,features_list,window,min_periods):
    for feature in features_list:
        df[f"past_{feature}"] = df[feature].rolling(window=window,min_periods=min_periods).sum()
    return df

# past season sum of the feature
